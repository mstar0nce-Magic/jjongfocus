...
async function refresh(){if(refreshing){pending=true;return}refreshing=true;try{const view=activeView();if(view==='calendar'){window.JjongCalendar?.render?.()}else if(view!=='completed'){const search=document.querySelector('#search');if(search)search.dispatchEvent(new Event('input',{bubbles:true}))}await new Promise(r=>requestAnimationFrame(()=>r()));}finally{refreshing=false;if(pending){pending=false;requestRefresh()}}}
patchIndexedDB();
