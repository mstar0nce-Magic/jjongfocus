...
function bindEnter(){
  const dialog=document.querySelector('#diaryPasswordDialog');
  const submit=document.querySelector('#diaryPasswordSubmit');
  if(!dialog||!submit||dialog.dataset.enterBound==='1')return false;
  dialog.dataset.enterBound='1';
  ...
  if(form)form.addEventListener('submit',e=>{e.preventDefault();submit.click()});
  return true;
}
...
function boot(){bindEnter();addReset()}
boot();
